...
// Ambil username (asumsi Anda sudah memiliki session ini)
$username = $_SESSION['username'] ?? 'User';

// ----------------------------------------------------
// 1. AMBIL SEMUA VENDOR UNTUK FILTER DROPDOWN
// ----------------------------------------------------

$vendors = [];
$query_vendors = "SELECT vendorID, vendorName FROM vendors ORDER BY vendorName ASC";
$result_vendors = mysqli_query($connection, $query_vendors);
if ($result_vendors) {
...
// ----------------------------------------------------
// 2. LOGIKA FILTER, SEARCH, & SORT BARU
// ----------------------------------------------------

// Inisialisasi variabel filter dan sort dari GET request
$search_term = $_GET['search'] ?? '';
$filter_vendor_id = $_GET['vendor'] ?? '';
$sort_by = $_GET['sort_by'] ?? 'name_asc'; // Default: Nama A-Z
$where_clauses = [];
$bind_params = '';
$bind_values = [];
...
// C. Logika Sort By
$order_by_clause = "p.productID ASC"; // Default jika $sort_by tidak dikenal
...
// Periksa apakah ada parameter yang perlu di-bind (untuk Prepared Statement)
